...
[rowsW, colsW, cameraPointsW, projectorPointsW, ~] = decodeMPS(mpsDirectory, params);

% rowsInt = round(rows);
% colsInt = round(cols);

[~,~,rowsBlockW] = histcounts(round(rowsW), round((params.screenHeight/params.blockSize)/params.projectorResolutionDecimationFactor));
[~,~,colsBlockW] = histcounts(round(colsW), round((params.screenWidth/params.blockSize)/params.projectorResolutionDecimationFactor));
...
%% Cutting projectorPoints and cameraPoints to fit ROI

PP = [projectorPointsW(xr(1):xr(2)), projectorPointsW((size(rows,2) + yr(1)) : (size(rows,2) + yr(2)))]; 
CP = [cameraPointsW(xr(1):xr(2)), cameraPointsW((size(rows,2) + yr(1)) : (size(rows,2)+yr(2)))]; 

%%
tformProjector2Camera = fitgeotrans(PP, CP, 'projective');
outputView = imref2d([size(rowsW((yr(1) : yr(2)), :),1), size(rowsW(:,(xr(1):xr(2))) ,2)]);
...
whiteImageCamera = whiteImageCamera(yr(1):yr(2), xr(1):xr(2));
