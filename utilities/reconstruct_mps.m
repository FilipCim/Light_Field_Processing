% Script for reconstructing one projector multiple camera recordings using
% MPS patterns

clear all;
close all;

pause(0.01); % Required to close all images.


%% ADD PATHS
% Add paths to directories containing required functions.
de_bruijn_functions = './utilities/DeBruijn';
geometric_functions = './utilities/Calibration Geometric';
phase_shift_functions = './utilities/Phase Shift';
mps_functions = './utilities/Multiple Phase Shift';
reg_functions = './utilities/3D Registration';


p = path;
if isempty(strfind(p, geometric_functions)); path(geometric_functions, path); end;
if isempty(strfind(p, hessian_functions)); path(hessian_functions, path); end;
if isempty(strfind(p, phase_shift_functions)); path(phase_shift_functions, path); end;
if isempty(strfind(p, mps_functions)); path(mps_functions, path); end;
if isempty(strfind(p, point_cloud_functions)); path(point_cloud_functions, path); end;
...
data_dirs = { ...
    '/home/ivan/Development/Codes/SlideshowCapture/imagesCamera/InFocus MPS Images', ... % 1
    };
...
%     [cam, prj] = gc_load_geometry(calibration_xml_file);
    ...
    for i = 1 : num_cam
        ...
        %% GET CALIBRATION DATA FOR ACTIVE CAMERA
        % Select appropriate camera and projector from preloaded calibration
        % data.
        
%         [w1, h1, fx1, fy1, cx1, cy1, K1, P1] = gc_load_named_geometry([cam; prj], camera_names{i});
%         [w2, h2, fx2, fy2, cx2, cy2, K2, P2] = gc_load_named_geometry([cam; prj], projector_name);
        ...
        if ~isempty(P_col)
            projector_col = P_col(-1 ~= lut) * 1920;
        else
            projector_col = [];
        end
        
        if ~isempty(P_row)
            projector_row = P_row(-1 ~= lut) * 1080;
        else
            projector_row = [];
        end
