...
%%
% %% ESTIMATE GAMMA CORRECTION CURVE
% 
% imagesCameraDirectoryPath = fullfile(currentDirectory,'/imagesCamera/grayscaleImages/');
% params.captureEveryNthImage = 8;
% 
...
%     imageCamera = imread([imagesCameraDirectoryPath, imageFilenames{i}]);
%     
%     outputIntensity(i) = sum(imageCamera(:));
%     inputIntensity = [0:params.captureEveryNthImage:2^params.bitDepth-1];
%     
%     %     differentGrayscaleLevelImages(:,:,i) = imwarp(imageCamera, tformCamera2Projector, 'OutputView',outputView);
%     differentGrayscaleLevelImages(:,:,i) = imageCamera;
%     
%     
%     %     figure(1)
%     %     imagesc(differentGrayscaleLevelImages(:,:,i))
%     %     drawnow
%     
% end
% 
% outputIntensityNormalized = outputIntensity./max(outputIntensity);
% inputIntensityNormalized = inputIntensity./max(inputIntensity);
% 
% figure, plot(inputIntensityNormalized, outputIntensityNormalized)
% 
% 
% %%
% 
% p = polyfit(log(inputIntensityNormalized(2:end)), log(outputIntensityNormalized(2:end)), 1);
% 
% hold on
% plot(inputIntensityNormalized, [exp(p(2)).*(inputIntensityNormalized.^p(1))])
% 
% 
% 
% % %% EXAMPLE OF SIMPLE LOG REGRESSION
% %
% % input = [0:0.1:1];
% % output = 2*[0:0.1:1].^1.78;
% %
% % figure
% % plot(input,output)
% %
% % p = polyfit(log(input(2:end)), log(output(2:end)), 1);
% %
% % hold on
% % plot(input, [(exp(p(2)).*(input.^p(1)))+0.0001])
% %
% % % p = polyfit(log(output(2:end)), log(input(2:end)), 1);
% %
% % plot(input, [((1/exp(p(2))).*(output.^(1/p(1))))])
% 
